...
public class smsdata {
    ...
    // *******************************************************************************************************
    public void txtToSmsPdu(String testo) {
        ...
        //System.out.println("bitset="+ccc[0]);
        ...
        /* StringBuilder binary = new StringBuilder();
         ...
         System.out.println("'" + testo + "' to binary: " +"\n"+ binary);*/

        //for(int c=0;c<testo.length(); c++)System.out.print("0x"+ UnicodeFormatter.byteToHex(ccc[c])+"\t\t");
        int l = 0;
        // System.out.println("\n"+"ascii is"+"\n");
        // for(int c=0;c<testo.length(); c++)System.out.print("ccc["+c+"]=="+ccc[c]+"\t");
        for (int i = 0; i < testo.length(); i++) {
            ...
            for (int subI = 0; subI < 7; subI++) {
                ...
                if ((ccc[i] & (1 << subI)) != 0) {
                    bs.set(l);
                    // System.out.println("bit seted at"+l);
                }
            }
        }
        l++;
        ...
        for (int i = 0; i < ll; i++) {
            ...
            for (int subI = 0; subI < 8; subI++) {
                if ((l + 1) > (i * 8 + subI)) {
                    //System.out.println("i&subI======"+i+","+subI);// should be less then last 1 in bs
                    //System.out.println("bs.get======"+bs.get(i*8+subI));
                    if (bs.get(i * 8 + subI)) {

                        b[i] |= (byte)(1 << subI);
                        Integer.toBinaryString(b[i]);
        ...
        pduTxt = new String("");
        for (int i = 0; i < ll; i++) {
            String str1 = Integer.toHexString(b[i]); // convert # in string
            if (str1.length() < 2) {
                str1 = '0' + str1;
            }
    ...
    public String txtToSmsPdu1(String testo) {
        // first of all set the msg lenght
        pduTxtLen = Integer.toHexString(testo.length());
    ...
    //if no. is +918104405668
    //converted into 191840046586
    ...
    // ********************************
    public String smsPduToTxt(String s) {
